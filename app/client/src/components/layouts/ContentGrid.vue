<template>
  <div style="height: 100%; width: 100%" @dragover="onDragover" @drop="onDropTab">
    <v-tabs v-model="tab" height="42px">
      <v-tab v-for="item in tabs" :key="item.name" style="font-size: 1em" draggable @dragstart="onDragStart($event, item)">
        {{ item.name }}
      </v-tab>
    </v-tabs>
    <div :class="'tab-wrapper ' + theme" >
      <v-tabs-items class="tab-item-container" v-model="tab">
        <v-tab-item v-for="item in tabs" :key="item.name" style="height: 100%; width: 100%">
          <component :is="item.content" :params="item.params" :theme="theme" :activate.sync="item.activate"></component>
        </v-tab-item>
      </v-tabs-items>
    </div>
  </div>
...
export default {
  ...
  mounted() {
    var tab_info = this.$common.store.getProperty("app.tab_info");
    this.onTabs(tab_info);
    console.log("mounted");
  },
  destroyed() {
